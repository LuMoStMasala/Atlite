...
pvgeneration = pd.DataFrame(index=cutout.meta.time.values)
windgeneration = pd.DataFrame(index=cutout.meta.time.values)

# Add generation time series from real installed capacities from
# Anlagenregister in 2012
pvgeneration['Anlagenregister'] = \
    cutout.pv(panel="CSi",
              orientation='latitude_optimal',
              layout=solar_layout
              ).to_pandas().iloc[0]
    
windgeneration['Anlagenregister'] = compute_wind_output().to_pandas().iloc[0]
...
capacityFactors_WIND = cutout.wind(turbine='Vestas_V112_3MW',
                                   capacity_factor=True)
#%%
# =============================================================================
...
for key in layout_variation_PV:
    pvgeneration[key] = cutout.pv(panel="CSi",
                                orientation='latitude_optimal',
                                layout=layout_variation_PV[key]).to_pandas().iloc[0]
# Add reference (OPSD) time-series
pvgeneration['OPSD'] = opsd['DE_solar_generation_actual']

# Plot weekly averages
pvgeneration.divide(1000).resample('1W').mean().plot(figsize=(10,6))
...
# <codecell>
# =============================================================================
# Wind Layout variation
# =============================================================================
layout_variation_WIND = {
    'highest CF, 50 MW':
        synthesize_layout(solar_layout, capacityFactors_WIND,
                          P_TOTAL_WIND,
                          'CF highest',
                          cellLimit=50),
    'highest CF, 100 MW':
...
# Compute generation data for all capacity layout variations
for key in layout_variation_WIND:
    windgeneration[key] = cutout.wind(turbine='Vestas_V90_3MW',
                                      layout=layout_variation_WIND[key],
                                      show_progress=False).to_pandas().iloc[0]

# Add reference (OPSD) time-series
pvgeneration['OPSD'] = opsd['DE_solar_generation_actual']

# Plot weekly averages
windgeneration.divide(1000).resample('1W').mean().plot(figsize=(10,6))
plt.ylabel("Feed-In [GW]")
plt.title('Wind time-series (weekly avrg.) in Germany. Total Wind Capacity'+ \
          ' P_tot_wind: ' + str(round(P_TOTAL_WIND/1e3,1)) + ' GW')
plt.tight_layout()


# =============================================================================
# Get total energy generated by different layout variations for PV and Wind
# =============================================================================
energy = dict()
energy['PV'] = pd.DataFrame(index={'Total Energy [TWh]'})
for variation in pvgeneration.columns:
    energy['PV'][variation] = pvgeneration[variation].sum() / 1e6
    
energy['Wind'] = pd.DataFrame(index={'Total Energy [TWh]'})
for variation in windgeneration.columns:
    energy['Wind'][variation] = windgeneration[variation].sum() / 1e6




# compare = pd.DataFrame({"atlite":wind['total'].to_pandas().iloc[0],
#                         # "< 400 kW":wind['< 400.0 kW'].to_pandas().iloc[0],
#                         # "< 1600 kW":wind['< 1600.0 kW'].to_pandas().iloc[0],
#                         # "< 2500 kW":wind['< 2500.0 kW'].to_pandas().iloc[0],
...
# laySum = turbine_categories[0]['layout']
# laySum.data = np.zeros(shape=layout.data.shape)
# summ = 0
# for cat in turbine_categories:
#     laySum.data = laySum.data + cat['layout'].data
#     summ += cat['layout'].data.sum()